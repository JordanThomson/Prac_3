...
int main(void)
{
  ...
  while (1)
  {
	...
	// Read ADC value from the potentiometer (PA6)
	uint32_t adc_value = pollADC();

	// Convert ADC value to a string
	char adc_string[20];  // Adjust the size as needed
	sprintf(adc_string, "ADC Value: %lu", adc_value);
...
void EXTI0_1_IRQHandler(void)
{
	...
	uint32_t current_time = HAL_GetTick();
	    ...
	    if ((current_time - last_button_press_time) >= 100) {
	        if (LL_EXTI_IsActiveFlag_0_31(LL_EXTI_LINE_0)) { // If the line triggered the interrupt
	            LL_EXTI_ClearFlag_0_31(LL_EXTI_LINE_0);  // Clear interrupt flags for EXTI0 (PA0)

	            // Toggle LED7 delay frequency when Button0 is pressed
	            if (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0)) {
	                if (delay_t == 500) {
	                    delay_t = 1000;  // Change to 1 Hz (1000ms)
	                } else {
	                    delay_t = 500;  // Change to 2 Hz (500ms)
	                }

	            }

	            // Update the button press for debouncing
...
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
	//Writes to the LCD, doesnt print null characters
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	//Initiates the ADC conversion using the HAL library
	HAL_ADC_Start(&hadc);

	// Blocks the unitl the ADC conversion is complete
	HAL_ADC_PollForConversion(&hadc, HAL_MAX_DELAY);

	// Retrieves ADC value from the HAL library
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	//CCR = (ADC*ARR)/4095
	uint32_t ccrValue = (adc_val * 47999) / 4095;
	return ccrValue;
}
